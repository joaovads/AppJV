        ...
        for day in week:
            if day == 0: 
                ...
                if day in aulas_dict:
                    ...
                    html_code += f"<td style='border:1px solid #bae6fd; padding:5px; background-color:#ffffff !important; vertical-align:top; height:80px;'><strong style='color:#0f172a;'>{day}</strong><div style='margin-top:5px;'>{temas}</div></td>"
                else: 
                    html_code += f"<td style='border:1px solid #bae6fd; padding:5px; background-color:#ffffff !important; vertical-align:top; height:80px;'><strong style='color:#64748b;'>{day}</strong></td>"
        html_code += "</tr>"
    html_code += "</table></div>"
    return html_code
...
# ==========================================
    ...
    # SETUP DE CORES SEGURO E ANTI-FALHAS
    if modo == "Escuro":
        bg_color = "#0e1117"
        text_color = "#f8fafc"
        metric_bg = "#1e293b"
    ...
    # INJEÇÃO CSS PROFUNDA (CORRIGE TUDO O QUE ESTAVA INVISÍVEL)
    st.markdown(f"""
    <style>
    ...
    /* FORÇAR A COR DE TODOS OS TEXTOS (Sobrescreve o Streamlit Nativo) */
    p, span, div, label, h1, h2, h3, h4, h5, h6, li {{
        color: {text_color};
    }}
    
    /* EXCEÇÃO 1: ABAS (Para não sumir o texto) */
    button[data-baseweb="tab"] p, button[data-baseweb="tab"] span {{
        color: {text_color} !important;
        font-weight: 500 !important;
    }}
    
    /* EXCEÇÃO 2: CAIXAS DE SELEÇÃO, INPUTS, TEXTAREA E UPLOAD (Para não ficarem pretas) */
    ...
    [data-baseweb="select"] span, [data-baseweb="input"] input, [data-baseweb="textarea"] textarea {{
        color: {text_color} !important;
        -webkit-text-fill-color: {text_color} !important;
    }}
    
    /* EXCEÇÃO 3: CAIXA DO CHAT DA IA */
    ...
    [data-testid="stChatInput"] textarea {{ color: {text_color} !important; -webkit-text-fill-color: {text_color} !important; }}
    
    /* EXCEÇÃO 4: TODOS OS BOTÕES DE AÇÃO AZUIS */
    ...
    /* Forçar texto branco dentro dos botões azuis */
    ...
    /* EXCEÇÃO 5: TABELAS CINZAS */
    table {{ width: 100%; border-collapse: collapse; margin-bottom: 1rem; border-radius: 8px; overflow: hidden; }}
    th {{ background-color: #cbd5e1 !important; color: #0f172a !important; padding: 10px; border: 1px solid #94a3b8 !important; font-weight: bold; text-align: left; }}
    td {{ background-color: #f1f5f9 !important; color: #0f172a !important; padding: 10px; border: 1px solid #cbd5e1 !important; }}
    ...
    [data-testid="stMetricLabel"] p {{ color: #64748b !important; font-weight: bold !important; }}
    
    /* MENU LATERAL (ESTILO ESTRATÉGIA) */
    ...
    if user_settings.get('foto_perfil') and os.path.exists(user_settings['foto_perfil']):
        st.sidebar.markdown(f'<img src="data:image/jpeg;base64,{base64.b64encode(open(user_settings["foto_perfil"], "rb").read()).decode()}" class="profile-img">', unsafe_allow_html=True)
